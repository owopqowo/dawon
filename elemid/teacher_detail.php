<? include $_SERVER["DOCUMENT_ROOT"]."/dawon/include/header.php" ?>

<!-- Container :: teacher_info -->
<div id="container" class="teacher_info">

	<!-- location -->
	<div class="location_wrap">
		<div class="section">
			<ul class="sub_menu">
				<li class="on"><a href="teacher.php">전체선생님</a></li>
				<li><a href="timetable.php">강의 시간표</a></li>
				<li><a href="briefing.php">설명회</a></li>
			</ul>
			<ul class="location">
				<li>HOME</li>
				<li>초중등관</li>
				<li>초중등 본관 선생님</li>
			</ul>
		</div>
	</div>
	<!-- location -->

	<!-- content -->
	<div class="gray_cont">
		<div class="section">
			<h2 class="title">초중등 본관 선생님</h2>

			<!-- tab menu -->
			<ul class="tab_menu w50">
				<li class="on"><a href="">중등</a></li>
				<li><a href="">초등</a></li>
				<li><a href="">입시</a></li>
				<li><a href="">컨설팅</a></li>
			</ul>
			<!-- //tab menu -->

			<!-- visual info -->
			<div class="visual_info">
				<div class="visual">
				</div>
				<div class="info">
					<div class="name">
						<div class="img_wrap">
							<img src="../images/teacher_img_01.jpg" alt="" />
						</div>
						<h4>전재림 선생님</h4>
						<p>한국사, 사회선생님</p>
					</div>
					<div class="list">
						<strong>[이력]</strong>
						<ul class="list_type_01">
							<li>현) 메가스터디 온/오프라인</li>
							<li>현) 대치 에스원 사회탐구 전문학원 부원장</li>
							<li>현) EBSI</li>
							<li>현) 비타에튜</li>
						</ul>
						<strong>[저서]</strong>
						<ul class="list_type_01">
							<li>2012~2017 저자 직강 시리즈3 (총45권)</li>
							<li>이기적IN 한국사능력검정시험 고급.중급 시리즈</li>
							<li>2017 마더텅 한국사, 동아시아사, 세계사 검수</li>
						</ul>
					</div>
				</div>
			</div>
			<!-- //visual info -->

			<!-- 수업특징 -->
			<div class="white_box">
				<h3 class="sub_title">전재림 선생님 수업의 특징</h3>
				<ul class="character">
					<li>
						<div class="img_wrap">
							<img src="../images/ico_main_10.png" alt="" />
						</div>
						<h4>흐름을 한눈에<br />판서한판</h4>
						<p>전체 흐름과 핵심을 한눈에 보는 <br />개념 구조판 판서를 일일이 받아 쓰는 <br />시간까지도 아낀 구성과 사건</p>
					</li>
					<li>
						<div class="img_wrap">
							<img src="../images/ico_main_12.png" alt="" />
						</div>
						<h4>부담을 줄여요<br />스토리로 재미있게</h4>
						<p>최선이 아닌 최고가 되고자 <br />노력하는 입시성공의 동반자</p>
					</li>
					<li>
						<div class="img_wrap">
							<img src="../images/ico_main_06.png" alt="" />
						</div>
						<h4>킬러에 강한<br />세기특강& 10지선다</h4>
						<p>킬러문항인 세기와 연도 구분을 올킬 <br />10지선다 다중답안으로 함정선지 공략 <br />1문제로 2~3문제를 푸는 효과</p>
					</li>
				</ul>
			</div>
			<!-- //수업특징 -->

			<!-- 수업진행 -->
			<div class="white_box">
				<h3 class="sub_title">수업진행 및 학생관리</h3>
				<div class="box">

				</div>
			</div>
			<!-- //수업진행 -->

			<!-- 수업교재 구성 -->
			<div class="white_box">
				<h3 class="sub_title">
					수업교재 구성
					<span class="desc">한국사,동아시아사, 세계사 1등급으로 이끄는 고아름입니다.</span>
				</h3>
				<div class="box">
					<img src="../images/test_book.jpg" alt="수업교재" />
				</div>
			</div>
			<!-- //수업교재 구성 -->

			<!-- 커리큘럼 -->
			<div class="white_box">
				<h3 class="sub_title">
					전재림 커리큘럼
					<span class="desc">한국사,동아시아사, 세계사 1등급으로 이끄는 고아름입니다.</span>
				</h3>
				<div class="box">
					<img src="../images/test_curriculum.jpg" alt="수업교재" />
				</div>
			</div>
			<!-- //커리큘럼 -->

			<!-- 강좌소개 -->
			<div class="white_box">
				<h3 class="sub_title">강좌소개</h3>
				<div class="video">
					<iframe frameborder="0" allowfullscreen="" src="https://www.youtube.com/embed/1uiDdXEJeSQ" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"></iframe>
				</div>
			</div>
			<!-- //강좌소개 -->

			<!-- 프로모션 정보 -->
			<div class="white_box">
				<h3 class="sub_title">전재림 프로모션 정보</h3>
				<div class="banner">
					<img src="../images/test_banner.jpg" alt="banner" />
					<img src="../images/test_banner.jpg" alt="banner" />
				</div>
			</div>
			<!-- //프로모션 정보 -->

			<!-- 예약 -->
			<ul class="reserve">
				<li>
					<a href="javascript:;">
						<h3 class="sub_title">이과정규</h3>
					</a>
					<div class="reserve_box">
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="timetable_reserve.php" class="btn_type_02">예약하기</a>
							</div>
						</div>
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="" class="btn_type_02 end">마감완료</a>
							</div>
						</div>
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="timetable_reserve.php" class="btn_type_02">예약하기</a>
							</div>
						</div>
					</div>
				</li>
				<li>
					<a href="javascript:;">
						<h3 class="sub_title">문과정규</h3>
					</a>
					<div class="reserve_box">
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="timetable_reserve.php" class="btn_type_02">예약하기</a>
							</div>
						</div>
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="timetable_reserve.php" class="btn_type_02">예약하기</a>
							</div>
						</div>
					</div>
				</li>
				<li>
					<a href="javascript:;">
						<h3 class="sub_title">클리닉</h3>
					</a>
					<div class="reserve_box">
						<div class="program">
							<div class="img_wrap">
								<span><img src="../images/test_person.jpg" alt=""></span>
							</div>
							<div class="info">
								<div class="info_box name">
									<span class="teacher_name">전재림 선생님</span>
									<span class="cate">내신</span>
									<span class="cate">수능</span>
								</div>
								<h4>수능국어 개념 집중반 문학+비문학</h4>
								<p>학생별 맞춤 데일리 과제 제공<br>배운 내용에 대한 확인테스트로</p>
								<span class="line"></span>
								<div class="info_box period">
									<span class="time"><strong>시간</strong>18:00 -19:30</span>
									<span class="date"><strong>개강일</strong>2018.10.30(화)</span>
								</div>
								<a href="timetable_reserve.php" class="btn_type_02">예약하기</a>
							</div>
						</div>
					</div>
				</li>
			</ul>
			<!-- //예약 -->

		</div>
	</div>
	<!-- //content -->

</div>
<!-- //Container :: teacher_info -->

<? include $_SERVER["DOCUMENT_ROOT"]."/dawon/include/footer.php" ?>
